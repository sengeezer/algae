...
type CatalogBrowseDetailPageProps = {
...
export function CatalogBrowseDetailPage({
  algorithms,
  backHref,
  backLabel,
  description,
  ...
  title,
  typeLabel,
}: CatalogBrowseDetailPageProps) {
  const visibleHighlightChips = Array.from(new Set(highlightChips.filter(Boolean)));
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 py-4 sm:py-8">
        <section className="glass-panel rounded-[36px] px-5 py-6 sm:px-8 sm:py-8">
          ...
          <div className="mt-6 flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              {visibleHighlightChips.length > 0 ? (
                <div className="mt-4 flex flex-wrap gap-2">
                  {visibleHighlightChips.map((chip) => (
                    <span
                      key={chip}
        ...
        <section className="glass-panel rounded-[28px] p-5 sm:p-6">
          <div className="flex flex-col gap-3 border-b border-black/8 pb-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-[var(--muted)]">Shelf</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-[var(--foreground)]">
                Scan the full reference set
              </h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-[var(--muted)]">
              These route-level shelves keep the browse experience stable at catalog scale while each reference still links out to the full algorithm page.
            </p>
          </div>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {algorithms.map((algorithm) => (
              <article
                key={algorithm.slug}
                className="rounded-[24px] bg-white/70 p-5 transition hover:-translate-y-0.5 hover:shadow-lg"
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {algorithm.dataStructures.slice(0, 4).map((structure) => (
                    <span key={structure} className="pill rounded-full px-3 py-1 text-xs font-medium">
                      {structure}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-5 flex items-center justify-between gap-3 border-t border-black/10 pt-4 text-sm text-[var(--muted)]">
                  <span>{algorithm.interviewFrequency} interview frequency</span>
                  <Link
